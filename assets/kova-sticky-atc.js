/**
 * KÖVA Sticky ATC Controller
 * Shows the sticky bar when the main ATC button is not in sight.
 */
document.addEventListener('DOMContentLoaded', function () {
    const stickyAtc = document.querySelector('.kova-sticky-atc');
    const mainAtcBtn = document.querySelector('.kova-atc-btn');

    if (!stickyAtc || !mainAtcBtn) return;

    // Function to handle the actual ATC action when sticky button is clicked
    const stickyBtn = stickyAtc.querySelector('.kova-sticky-atc-btn');
    if (stickyBtn) {
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    stickyAtc.classList.remove('kova-sticky-atc--visible');
                } else {
                    ...
                    if (window.scrollY > entry.boundingClientRect.top + window.scrollY) {
                        stickyAtc.classList.add('kova-sticky-atc--visible');
                    } else {
                        stickyAtc.classList.remove('kova-sticky-atc--visible');
                    }
                }
